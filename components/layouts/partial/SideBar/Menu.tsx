    ...
    <div className="mt-16 flex flex-col gap-2 w-full">
      {
        MENULIST.map((item: IMenu, index: number) => (
          <div key={index} className={"px-6 py-2 flex items-center gap-2 hover:cursor-pointer hover:bg-[#F0F3FB] rounded-[5px] transition duration-500 ease-in-out " + (pathname == item.link ? "bg-[#e6f3ff]" : "")} onClick={() => push(item.link)}>
