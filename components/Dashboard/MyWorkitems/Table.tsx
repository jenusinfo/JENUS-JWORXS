...
export const Table = () => {
  ...
  const { data, curPageNumber, setCurPageNumber, handleGetWorkitems, loading } = useWorkitem()
            ...
            <tbody className="text-sm">
              {
                data
                  ...
                  .map((inbox: IInbox, index: number) => (
                    <tr key={index} className="border-b border-gray-200 odd:bg-gray-100 even:bg-white hover:cursor-pointer hover:scale-[1.01] transition duration-500">
                      <td className="py-3">
                        <div className="flex justify-center">
                          <IoEllipsisVerticalSharp />
                        </div>
