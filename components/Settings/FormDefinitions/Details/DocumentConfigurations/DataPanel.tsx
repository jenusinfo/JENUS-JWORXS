...
import DocumentConfigurationModal from "./Modal";
import { useDocumentConfiguration } from "providers/settings/FormDefinitions/Details/DocumentConfigurationProvider";

const DocumentConfigurationsDataPanel = () => {

	const { documentConfigurations: data, setInfo, setCurIndex, handleDelete } = useDocumentConfiguration()
	const [isOpen, setIsOpen] = useState(false)

	return (
		<div className="p-4 bg-white shadow-md">
			<div className="flex justify-between items-center">
				<Text text="Document Configurations" size={16} weight="600" color="#275E93" className="border-b-2 border-blue-500" />
				<button className="text-white bg-[#2454de] hover:bg-blue-500 transition-all duration-400 rounded-[4px] px-5 py-2.5 h-fit" onClick={() => { setIsOpen(true); setInfo({}); setCurIndex(-1); }}>+ Create a Document Configuration</button>
			</div>
				...
				<tbody className="text-sm">
					{
						data.map((inbox: any, index: number) => (
							<tr key={index} className="border-b border-gray-200 hover:cursor-pointer hover:bg-gray-100 transition-all duration-500">
								...
								<td className="px-2">{inbox.Caption}</td>
								<td className="px-2">{inbox.EfsUrl}</td>
			...
			<DocumentConfigurationModal isOpen={isOpen} handleClose={() => setIsOpen(false)} />
		</div>
	)
}

export default DocumentConfigurationsDataPanel