...
import FormDefinitionsSectionModal from "./Modal";
...
const SectionsDataPanel = () => {

	const { interviewSections: data, setInfo, setCurIndex, handleDelete } = useSection()
	const [isOpen, setIsOpen] = useState(false)

	return (
		<div className="p-4 bg-white shadow-md">
			<div className="flex justify-between items-center">
				<Text text="Sections" size={16} weight="600" color="#275E93" className="border-b-2 border-blue-500" />
				<button className="text-white bg-[#2454de] hover:bg-blue-500 transition-all duration-400 rounded-[4px] px-5 py-2.5 h-fit" onClick={() => { setIsOpen(true); setInfo({}); setCurIndex(-1); }}>+ Create a Section</button>
			</div>
			...
			<FormDefinitionsSectionModal isOpen={isOpen} handleClose={() => setIsOpen(false)} />
		</div>
	)
}
...
const DraggableRow = ({ index, inbox, moveRow, handleDelete, setIsOpen, setInfo, setCurIndex, arrIndex, setArrIndex }: any) => {
	...
	const { getInterviewSections } = useSection()
	...
	const [, drop] = useDrop({
		...
		async drop(item: any) {
			setArrIndex([])
			await http.post(`/Interview/Sections/Reorder?direction=${index-arrIndex[0]}&id=${inbox.Id}`, {})
	...
	return (
		...
		>
			...
			<td className="px-2">{inbox.Label}</td>
			<td className="px-2">{inbox.IsRepeatable ? 'Yes' : 'No'}</td>
...
export default SectionsDataPanel