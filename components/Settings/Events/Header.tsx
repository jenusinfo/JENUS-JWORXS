...
const EventsHeader = () => {
    ...
    const { setCurIndex, setInfo, events, search, setSearch } = useEvents()
    const [isOpen, setIsOpen] = useState(false)

    return (
        <div className="flex justify-between border-b border-[#DEDFEA] pb-2">
            <div className="flex flex-col gap-3">
                <Text text="Events" size={28} weight="700" />
                <p className="text-sm">{events?.filter((each: any) => each.IsActive == true).length} Active Events</p>
            </div>
