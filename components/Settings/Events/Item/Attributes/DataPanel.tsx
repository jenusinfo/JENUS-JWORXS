...
const EventAttributesDataPanel = () => {

    const { eventAttributes: data, setInfo, setCurIndex, handleDelete } = useAttribute()
    const [isOpen, setIsOpen] = useState(false)

    return (
        <div className="p-4 bg-white shadow-md mt-8">
            <div className="flex justify-between items-center">
                <Text text="Attributes" size={16} weight="600" color="#275E93" className="border-b-2 border-blue-500" />
                <button className="text-white bg-[#2454de] hover:bg-blue-500 transition-all duration-400 rounded-[4px] px-5 py-2.5 h-fit" onClick={() => { setIsOpen(true); setInfo({}); setCurIndex(-1); }}>+ Create a Attribute</button>
            </div>
                    ...
                    <tbody className="text-sm">
                        {
                            data.map((inbox: any, index: number) => (
                                <tr key={index} className="border-b border-gray-200 hover:cursor-pointer hover:bg-gray-100 transition-all duration-500">
                                    ...
                                    <td className="px-2">{inbox.Type}</td>
...
export default EventAttributesDataPanel